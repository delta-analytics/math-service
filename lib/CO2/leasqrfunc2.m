function y = leasqrfunc2(x, p)
  global ratio_E_vs_E_filt;
  AHitr = load("-ascii", "HitranExtinktion2");
  grd = AHitr(:,1)';
  E = AHitr(:,2)';
  ...
  % fit amplitude
  E = fnl(E, p(7), p(8));  ## simulate nonlinearity by reducing amplitude according to fnl
  E = E * amp;  ## amp is only a fit parameter, not the mixing ratio!!!
  area_E = trapz(grd,E);
  % apply filter
